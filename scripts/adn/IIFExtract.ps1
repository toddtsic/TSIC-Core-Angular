...
function Consolidate-IifFiles {
    ...
    Write-Host "Found $($iifFiles.Count) file(s) to evaluate.`n" -ForegroundColor Gray

    try {
        # PASS 1: Build array of valid IIF files with sort order assigned
        $keywordOrder = @("Payments", "Credits", "TSIC-Fees", "MERCH-Fees", "CC-Fees", "Admin-Fees", "Retainers", "Checks")
        $validIifFiles = @()

        foreach ($file in $iifFiles) {
            Write-Host "Evaluating: $($file.Name)" -ForegroundColor Gray
            ...
            foreach ($line in $lines) {
                ...
                if ([string]::IsNullOrWhiteSpace($trimmed) -or $trimmed -eq "!ENDDATA") { continue }
                if ($trimmed -match '^!') { $hasHeaders = $true }
                else { $hasData = $true }
                if ($hasHeaders -and $hasData) { break }
            }

            # Only add to valid array if both headers and data found
            if ($hasHeaders -and $hasData) {
                # Determine sort order by keyword match
                $sortOrder = $keywordOrder.Count  # Default: end of list if no match
                for ($k = 0; $k -lt $keywordOrder.Count; $k++) {
                    if ($file.Name -like "*$($keywordOrder[$k])*") {
                        $sortOrder = $k
                        break
                    }
                }
                Write-Host "  [VALID] Sort order: $sortOrder" -ForegroundColor Green
                $validIifFiles += [pscustomobject]@{ File = $file; SortOrder = $sortOrder }
            }
            else {
                Write-Host "  [SKIP] Invalid (missing headers or data)" -ForegroundColor DarkGray
            }
        }

        if ($validIifFiles.Count -eq 0) {
            Write-Host "`nNo valid IIF files found." -ForegroundColor Yellow
            return $false
        }

        # PASS 2: Sort array by sort order
        $validIifFiles = $validIifFiles | Sort-Object SortOrder
        Write-Host "`nConsolidating $($validIifFiles.Count) file(s) in sorted order...`n" -ForegroundColor Cyan
        ...
        foreach ($fileObj in $validIifFiles) {
            $file = $fileObj.File
        ...
        foreach ($block in $consolidatedBlocks) {
            $consolidatedContent += $block
                # Delete existing consolidated file if it exists (force clean overwrite)
                if (Test-Path $OutputFilePath) {
                    Remove-Item $OutputFilePath -Force
                    Write-Host "Removed existing consolidated file" -ForegroundColor Gray
                }
        ...
        Write-Host "`nFiles Included (in order):" -ForegroundColor Gray
        foreach ($fileObj in $validIifFiles) {
            Write-Host "  [$($fileObj.SortOrder)] $($fileObj.File.Name)" -ForegroundColor Gray
        }
