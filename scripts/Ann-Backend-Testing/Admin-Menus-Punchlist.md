...
- **RESOLUTION (Todd go 2026-07-27, implemented)**: chips drag-reorderable exactly per sketch — `cdkDropList` with `cdkDropListOrientation="mixed"` (wrapping chip rows), `cdkDrag` per chip, `onChipDrop` clones + `moveItemInArray` + `.set()` (same mutation shape as `removeValue`); existing dirty-detection/Save/Reset unchanged. Grab cursor + placeholder fade + reduced-motion guard; preview clone styled in `_component-overrides.scss` (CDK appends it to `<body>`, outside encapsulation — house pattern per help-editor). Risk assessment on file: component fully isolated (own `/job-ddl-options` endpoint, no JobConfigService coupling); frontend-only; ng build green. Alphabetize button skipped (YAGNI).
- **Severity**: UX / Feature
- **Status**: FIXED — 2026-07-27, awaiting Todd verify (drag a Jersey Size chip, Save, reload) + Ann next pass
