...
- **✅ RESOLVED (Todd, 2026-07-29)** — the two real defects fixed; premise-based items reclassified:
  ...
  - **B — wrapping: DONE.** Added `overflow-wrap:anywhere; word-break:break-word;` to `.preview-body` and to the RTE editable area (`::ng-deep .e-rte-content .e-content`) so long content wraps instead of running off the edge.
