...
- **ATTEMPTED AND REVERTED (2026-07-27)**: CDK drag was implemented per sketch but **wrapping chip rows require `cdkDropListOrientation="mixed"`, whose sort strategy physically rearranges DOM nodes and then restores them from a sibling-snapshot on drop — racing Angular's `@for` re-render**. Order scrambled on the next change-detection pass (Todd repro'd twice: revert-on-Save, then again after a trackBy fix). All drag code + help mentions reverted at Todd's call rather than continuing to chase CDK internals pre-go-live. Note for future: the in-repo CDK drag successes (widget-editor, profile-editor, help-editor, schedule designers) are all single-axis vertical lists — default orientation, no wrap. Chips-that-wrap is the unsupported case.
- **Plan B (offered, pending Todd)**: deterministic ‹ › nudge buttons per chip (one-position moves, pure array splice, no CDK) — same Save/Reset flow, whack-a-mole-proof.
- **Severity**: UX / Feature
- **Status**: REVERTED — drag withdrawn 2026-07-27; plan B (nudge arrows) awaiting Todd go/no-go
