...
- **RESOLUTION (Todd, 2026-07-27)**: **toggles STAY on Teams — rosters are a team property**; the relocation half is declined. The real defect (bare "Adult"/"Player" labels) fixed in place, template-only: checkboxes stacked with descriptive labels ("Allow players to view their team roster" / "Allow coaches &amp; staff to view their team roster") + one-line `.field-help` each. Help copy corrected twice on Todd's review: club reps dropped (not gated by this flag), and a "tournament self-registered coaches" clause DELETED — it was extrapolated from a stale `AdultRegistrationService` invariant comment; **staff self-rostering onto teams no longer exists**. No DTO/service/payload changes; runtime consumers read the DB column and are untouched.
- **Severity**: UX
