<#
.SYNOPSIS
    Runs all schedule-related tests (games, standings, brackets).

.DESCRIPTION
    Validates schedule game filtering, W-L-T standings calculations,
    sport-specific sort order, goal diff capping, bracket grouping,
    champion detection, and bracket tree structure.
#>
...
Write-Host "    Schedule Tests" -ForegroundColor Cyan
Write-Host "    (games + standings + brackets)" -ForegroundColor Gray
Write-Host "  ========================================" -ForegroundColor Cyan
Write-Host ""

$output = dotnet test $testProject --filter "FullyQualifiedName~TSIC_Events.Schedule" --no-build --no-restore --verbosity normal 2>&1
$passed = @(); $failed = @()
