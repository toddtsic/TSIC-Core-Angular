<#
.SYNOPSIS
    Runs device management tests (shared by TSIC-Events + TSIC-Teams).

.DESCRIPTION
    Validates device registration, team subscription toggle, token swap,
    and subscription queries against InMemory DB.
#>
...
Write-Host "    Device Management Tests" -ForegroundColor Cyan
Write-Host "    (shared: TSIC-Events + TSIC-Teams)" -ForegroundColor Gray
Write-Host "  ========================================" -ForegroundColor Cyan
Write-Host ""

$output = dotnet test $testProject --filter "FullyQualifiedName~Mobile.Shared.Devices" --no-build --no-restore --verbosity normal 2>&1
$passed = @(); $failed = @()
