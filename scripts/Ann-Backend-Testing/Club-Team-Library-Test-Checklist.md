...
- [ ] D3. Every row has an Actions column with Edit and ONE removal button, and nothing about this event. Never registered anywhere: **Delete** (red on hover). Registered for another event: **Archive**. Registered for this event: **Archive** greyed and struck through; hovering shows **Registered for** and the event name, and clicking it shows the same as a toast. Archive and Delete never appear together.
...
- [ ] D4b. On Zz Test Beta (registered for an event): **Edit** › change the name to a different year, e.g. 2048 Blue, or change the grad year. A blue "Looks like a different team?" note appears with **Add 2048 Blue as a new team** and **No, I'm fixing Zz Test Beta**. Change only the level of play or the words in the name: no note. Click **Add … as a new team**: 2048 Blue is created, Zz Test Beta is untouched, and the modal offers **Archive Zz Test Beta** / **Keep both** (Archive is not offered when Zz Test Beta is registered for THIS event; the note says why). Keep both, then delete 2048 Blue.
