<#
.SYNOPSIS
    Runs team search filter tests (search/teams admin grid).

.DESCRIPTION
    Validates that the SearchTeamsAsync method in TeamRepository correctly
    filters teams by active status, club name, pay status, level of play,
    agegroup, LADT tree, waitlist status, CADT tree, and projects club rep
    info and financial totals accurately.

.EXAMPLE
    .\Team-Search-Tests.ps1
#>
...
Write-Host "    Team Search Tests" -ForegroundColor Cyan
Write-Host "    (search/teams admin grid)" -ForegroundColor Gray
Write-Host "  ========================================" -ForegroundColor Cyan
Write-Host ""

$output = dotnet test $testProject --filter "FullyQualifiedName~SearchTeams" --no-build --no-restore --verbosity normal 2>&1
$passed = @(); $failed = @()
...
Write-Host ""
if ($total -eq 0) {
    Write-Host "  No team search tests found yet - coming soon!" -ForegroundColor Yellow
