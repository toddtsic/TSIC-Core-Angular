...
- **Status**: Open — **needs Todd's decision on the intended semantics**
...
- **Severity**: UX / data-continuity — **not data loss** (legacy rows are intact in the DB); the cost is re-entry and risk of incomplete recruiting profiles. Scope (which fields, how many players) to be pinned by repro.
- **Status**: Open — mechanism partly **confirmed** (height), full field scope needs a repro pass
...
- **Status**: Open — **root cause confirmed, fix identified**
...
- **Status**: Open — root cause confirmed; **recommend Ann's option 2 (dependent field), NOT option 1 (remove checkbox)**
