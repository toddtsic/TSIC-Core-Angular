...
Write-Host "    Team Links Tests" -ForegroundColor Cyan
Write-Host "  ========================================" -ForegroundColor Cyan
Write-Host ""

$output = dotnet test $testProject --filter "FullyQualifiedName~TSIC_Teams.Links" --no-build --no-restore --verbosity normal 2>&1
$passed = @(); $failed = @()
