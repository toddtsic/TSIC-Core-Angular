using System;
using System.Collections.Generic;

namespace TSIC.Domain.Entities;

public partial class AdndataFromPhoenix
{
    public string? RegistrationId { get; set; }

    public double? AId { get; set; }

    public double? Active { get; set; }

    public double? AdnCc4 { get; set; }

    public DateTime? AdnCcexpDate { get; set; }

    public string? AdnInvoiceNo { get; set; }

    public double? AdnTransactionId { get; set; }

    public string? CheckNo { get; set; }

    public string? Comment { get; set; }

    public double? Createdate { get; set; }

    public double? Dueamt { get; set; }

    public string? LebUserId { get; set; }

    public double? Modified { get; set; }

    public double? Payamt { get; set; }

    public string? PaymentMethodId { get; set; }

    public string? Paymeth { get; set; }

    public string? PromoCode { get; set; }

    public string? TeamId { get; set; }

    public string? DiscountCodeAi { get; set; }

    public double? RegistrationAi { get; set; }

    public string? RegistrationCategory { get; set; }

    public string? RegistrationGroupId { get; set; }

    public string? RegistrationId2 { get; set; }

    public double? RegistrationTs { get; set; }

    public string? RoleId { get; set; }

    public string? UserId { get; set; }

    public string? FamilyUserId { get; set; }

    public double? BActive { get; set; }

    public double? BConfirmationSent { get; set; }

    public string? JobId { get; set; }

    public string? LebUserId3 { get; set; }

    public double? Modified4 { get; set; }

    public string? RegistrationFormName { get; set; }

    public string? AssignedTeamId { get; set; }

    public string? PaymentMethodChosen { get; set; }

    public double? FeeProcessing { get; set; }

    public double? FeeBase { get; set; }

    public double? FeeDiscount { get; set; }

    public double? FeeDiscountMp { get; set; }

    public double? FeeDonation { get; set; }

    public double? FeeLatefee { get; set; }

    public double? FeeTotal { get; set; }

    public double? OwedTotal { get; set; }

    public double? PaidTotal { get; set; }

    public string? CampsAttending { get; set; }

    public string? CustomerId { get; set; }

    public string? Honors { get; set; }

    public string? LeaguesAttending { get; set; }

    public string? PreviousCoach1 { get; set; }

    public string? PreviousCoach2 { get; set; }

    public string? Act { get; set; }

    public double? BBgcheck { get; set; }

    public double? BMedAlert { get; set; }

    public double? BScholarshipRequested { get; set; }

    public double? BTravel { get; set; }

    public double? BWaiverSigned1 { get; set; }

    public double? BWaiverSigned2 { get; set; }

    public double? BWaiverSigned3 { get; set; }

    public string? BackcheckExplain { get; set; }

    public string? BgCheckDate { get; set; }

    public string? CertDate { get; set; }

    public string? CertNo { get; set; }

    public string? ClassRank { get; set; }

    public string? ClubName { get; set; }

    public string? Gpa { get; set; }

    public double? GradYear { get; set; }

    public string? HealthInsurer { get; set; }

    public string? HealthInsurerGroupNo { get; set; }

    public string? HealthInsurerPhone { get; set; }

    public string? HealthInsurerPolicyNo { get; set; }

    public string? HeightInches { get; set; }

    public string? InsuredName { get; set; }

    public string? JerseySize { get; set; }

    public string? Kilt { get; set; }

    public string? MedicalNote { get; set; }

    public string? Position { get; set; }

    public string? SchoolTeamName { get; set; }

    public string? Psat { get; set; }

    public string? Region { get; set; }

    public string? RequestedAgegroupId { get; set; }

    public string? RoommatePref { get; set; }

    public string? Sat { get; set; }

    public string? SatMath { get; set; }

    public string? SatVerbal { get; set; }

    public string? SatWriting { get; set; }

    public string? SchoolGrade { get; set; }

    public string? SchoolName { get; set; }

    public string? Shoes { get; set; }

    public string? ShortsSize { get; set; }

    public string? SportAssnId { get; set; }

    public string? SportAssnIdexpDate { get; set; }

    public string? SportYearsExp { get; set; }

    public string? TShirt { get; set; }

    public string? UniformNo { get; set; }

    public string? VolChildreninprogram { get; set; }

    public string? Volposition { get; set; }

    public string? WeightLbs { get; set; }

    public string? NightGroup { get; set; }

    public string? DayGroup { get; set; }

    public string? AssignedAgegroupId { get; set; }

    public string? AssignedCustomerId { get; set; }

    public string? AssignedDivId { get; set; }

    public string? AssignedLeagueId { get; set; }

    public string? Assignment { get; set; }

    public string? SpecialRequests { get; set; }

    public string? Reversible { get; set; }

    public string? Gloves { get; set; }

    public string? Sweatshirt { get; set; }

    public string? DiscountCodeId { get; set; }

    public string? FiveTenFive { get; set; }

    public string? Threehundredshuttle { get; set; }

    public string? Fourtyyarddash { get; set; }

    public string? Fastestshot { get; set; }

    public double? BCollegeCommit { get; set; }

    public string? CollegeCommit { get; set; }

    public string? WhoReferred { get; set; }

    public string? StrongHand { get; set; }

    public string? ClubCoach { get; set; }

    public string? ClubCoachEmail { get; set; }

    public string? SchoolCoach { get; set; }

    public string? SchoolCoachEmail { get; set; }

    public string? SchoolActivities { get; set; }

    public string? HonorsAcademic { get; set; }

    public string? HonorsAthletic { get; set; }

    public string? OtherSports { get; set; }

    public string? HeadshotPath { get; set; }

    public string? SchoolLevelClasses { get; set; }

    public string? Height { get; set; }

    public string? MomTwitter { get; set; }

    public string? DadTwitter { get; set; }

    public string? MomInstagram { get; set; }

    public string? DadInstagram { get; set; }

    public string? Twitter { get; set; }

    public string? Instagram { get; set; }

    public string? ClubTeamName { get; set; }

    public string? Sweatpants { get; set; }

    public string? SkillLevel { get; set; }

    public double? BWaiverSignedCv19 { get; set; }

    public string? AdnSubscriptionId { get; set; }

    public string? AdnSubscriptionStatus { get; set; }

    public string? AdnSubscriptionStartDate { get; set; }

    public string? AdnSubscriptionBillingOccurences { get; set; }

    public string? AdnSubscriptionAmountPerOccurence { get; set; }

    public double? AdnSubscriptionIntervalLength { get; set; }

    public string? RegformId { get; set; }

    public string? Snapchat { get; set; }

    public double? BUploadedInsuranceCard { get; set; }

    public double? BUploadedVaccineCard { get; set; }

    public double? BUploadedMedForm { get; set; }

    public string? ModifiedMobile { get; set; }
}
