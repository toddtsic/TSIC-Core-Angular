...
///
/// Today this answers AMEX eligibility. AMEX is an allow-listed exception: it is offered only
/// when the job's customer (merchant account) has <c>Jobs.Customers.bAllowAmex = 1</c>, sitting
/// alongside that customer's Authorize.Net credentials. Fail-closed — an unknown job, a job with
/// no customer, or a customer whose flag is 0 resolves to <c>false</c> so we never offer a card
/// type the merchant can't settle.
///
...
{
    /// <summary>
    /// True iff the job's customer has bAllowAmex = 1. Fail-closed to false for a
    /// null/unknown job, a job with no customer, or a customer with the flag unset.
    /// </summary>
