...
public class CustomerJobRevenueRepository : ICustomerJobRevenueRepository
{
    ...
    /// <summary>
    /// <c>Jobs.year</c> as an int, or null when the stored value is not a plausible season.
    /// One definition, shared with the feeder-pace widget so that both reports place a job in
    /// the same season - see <see cref="JobSeasonNaming.ParseYear"/>.
    /// </summary>
    private static int? ParseJobYear(string? raw) => JobSeasonNaming.ParseYear(raw);

    /// <summary>
    /// The lineage key: the job name with its season designator removed. The boundary rules and
    /// the real customer data that produced them live on
    /// <see cref="JobSeasonNaming.StripSeasonToken"/>.
    /// </summary>
    /// <param name="year">
    /// Unused - the rule is about the SHAPE of a season designator in the name, not about this
    /// job's stored year. Kept in the signature because every call site already has it.
    /// </param>
    private static string BuildGroupKey(string jobName, int year)
    {
        _ = year;
        return JobSeasonNaming.StripSeasonToken(jobName);
    }
