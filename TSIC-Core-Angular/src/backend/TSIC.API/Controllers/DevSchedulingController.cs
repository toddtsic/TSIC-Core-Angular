...
public class DevSchedulingController : ControllerBase
{
    ...
    [HttpPost("bracket/auto-score-pool-job")]
    [ProducesResponseType<BracketDevActionResult>(200)]
    public Task<ActionResult<BracketDevActionResult>> BracketAutoScorePoolJob(CancellationToken ct) =>
        RunBracketToolAsync((jobId, userId) =>
            _bracketDevTools.AutoScorePoolJobAsync(jobId, userId, ct));

    /// <summary>POST /api/dev-scheduling/bracket/auto-score-round-job — score every ready
    /// bracket game in the event so winners advance one round across all flights.</summary>
    [HttpPost("bracket/auto-score-round-job")]
    [ProducesResponseType<BracketDevActionResult>(200)]
    public Task<ActionResult<BracketDevActionResult>> BracketAutoScoreRoundJob(CancellationToken ct) =>
        RunBracketToolAsync((jobId, userId) =>
            _bracketDevTools.AutoScoreBracketRoundJobAsync(jobId, userId, ct));

    /// <summary>POST /api/dev-scheduling/bracket/auto-score-pool-agegroup — score every unscored
    /// pool game in ONE agegroup. Each score fires job-wide seed resolution, so a reseeding
    /// tournament's championship agegroups reseed automatically.</summary>
    ...
    /// <summary>POST /api/dev-scheduling/bracket/auto-score-round-agegroup — score every ready
    /// bracket game in ONE agegroup so winners advance one round.</summary>
