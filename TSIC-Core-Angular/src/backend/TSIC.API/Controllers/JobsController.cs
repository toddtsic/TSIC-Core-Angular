...
public class JobsController : ControllerBase
{
    ...
    public async Task<ActionResult<JobMetadataResponse>> GetJobMetadata(string jobPath)
    {
        ...
        var response = new JobMetadataResponse
        {
            ...
            PlayerRegReleaseOfLiability = TextSubstitutionService.StripClickingConsentSentences(jobMetadata.PlayerRegReleaseOfLiability ?? string.Empty),
            PlayerRegCodeOfConduct = TextSubstitutionService.StripClickingConsentSentences(jobMetadata.PlayerRegCodeOfConduct ?? string.Empty),
            PlayerRegCovid19Waiver = TextSubstitutionService.StripClickingConsentSentences(jobMetadata.PlayerRegCovid19Waiver ?? string.Empty),
    ...
    public async Task<ActionResult<JobPulseDto>> GetJobPulse(string jobPath, CancellationToken ct)
    {
            ...
            if (string.Equals(User.GetJobPath(), jobPath, StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (regId.HasValue && !string.IsNullOrEmpty(role))
                {
                    ...
                    pulse = pulse with
                    {
                        ...
                        MyHasLiveArbSubscription = ctx.HasLiveArbSubscription,
                        ...
                        MyClubRepClubName = ctx.ClubRepClubName,
        ...
        // no-store: the body varies by auth (the My* fields) and by live config, so a
        // "closed" pulse must never be stored and replayed — that stale-snapshot replay
        // is what produced the intermittent "registration not currently open" toast on
        // a fully-open job (PL-009/PL-053). no-cache alone proved insufficient: entries
        // stored before it shipped were reused under heuristic freshness without ever
        // revalidating, so the browser never saw the corrected header.
        Response.Headers.Append("Cache-Control", "private, no-store, no-cache");
        Response.Headers.Append("Vary", "Authorization");
