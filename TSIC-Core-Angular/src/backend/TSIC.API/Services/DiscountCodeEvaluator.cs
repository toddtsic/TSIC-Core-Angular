...
public sealed class DiscountCodeEvaluatorService : IDiscountCodeEvaluator
{
    ...
    public async Task<decimal> EvaluateAsync(int discountCodeAi, decimal baseAmount)
    {
        ...
        var raw = rec.CodeAmount ?? 0m;
        if (raw <= 0m) return 0m;

        if (rec.BAsPercent)
        {
            // Explicit rounding strategy for currency consistency.
            var pct = raw / 100m;
            if (pct <= 0m) return 0m;
            var discount = baseAmount * pct;
            if (discount <= 0m) return 0m;
            return Math.Round(discount, 2, MidpointRounding.AwayFromZero);
        }

        // Fixed amount: cap at baseAmount to avoid over-discounting.
        return raw > baseAmount ? baseAmount : raw;
    }
}
