...
/**
 * "Quick Links" editor HTTP access (Director/SuperDirector/SuperUser).
 * Reads/writes the current (logged-in) job's landing-hero visibility flags —
 * the backend resolves the job from the JWT, so no jobId/jobPath is sent.
 * Partial PUTs let the UI save one toggle at a time.
 */
