...
export class RegistrationSearchService {
	...
	/** Sandbox-only: render for the first recipient, deliver for real to a single test inbox. */
	sendTestEmail(request: EmailTestSendRequest): Observable<EmailTestSendResponse> {
		return this.http.post<EmailTestSendResponse>(`${this.apiUrl}/batch-email/test-send`, request);
	}
