...
//
// The job picker populated the editor's "A specific job" scope, letting a SuperUser open and
// write any job's player form. That scope is gone: the editor now edits only the job resolved
// from the caller's JWT, via PUT profiles/current/form, which has no jobId to supply.
//
// GET profiles/jobs — the all-jobs enumeration behind this picker — is commented out in
// ProfileMigrationController. Commented line-wise, not wrapped in a block, because the
// original contains /** */ docblocks that cannot nest.
