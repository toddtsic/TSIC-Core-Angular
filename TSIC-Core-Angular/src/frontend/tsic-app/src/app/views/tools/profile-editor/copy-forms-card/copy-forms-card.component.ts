...
//
// The Copy Forms card seeded THIS job's player form from ANOTHER job. Its source picker is a
// cross-job read, so under the rule "a SuperUser may only read and write the job they are
// logged into" there is nothing left to keep — a copy card whose source must also be the
// current job does nothing.
//
// Not a capability loss: Job Clone already carries CoreRegformPlayer,
// PlayerProfileMetadataJson and JsonOptions forward to a new job, and shifts the grad-year
// option lists while doing it — which this card never did.
//
// Its endpoints are commented out in ProfileMigrationController (the actual control) and its
// service methods in ProfileMigrationService. Commented line-wise, not wrapped in a block,
// because the original contains /** */ docblocks that cannot nest.
