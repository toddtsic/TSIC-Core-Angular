...
/**
 * Guard for the register-player route.
 *
 * Checks two conditions in order:
 * 1. BRegistrationAllowPlayer — is player registration open at all?
 * 2. BPlayerRegRequiresToken — does this job require an invite token?
 *
 ...
 */
export const playerInviteGuard: CanActivateFn = async (route, state) => {
	...
	// 1. Registration not open at all
	if (!pulse.playerRegistrationOpen) {
		toast.show('Player registration is not currently open for this event.', 'warning', 6000);
		return router.createUrlTree([`/${jobPath}`]);
	}

	// 2. No token required — open registration, proceed
	if (!pulse.playerRegRequiresToken) {
		return true;
	}
	...
	if (!inviteRegId || !user.userId) {
		toast.show('Only accepted players with valid invitations may register for this event.', 'danger', 6000);
		return router.createUrlTree([`/${jobPath}`]);
	}
	...
	try {
		result = await firstValueFrom(
			http.get<{ allowed: boolean }>(`${environment.apiUrl}/player-invite/validate`, {
				params: {
