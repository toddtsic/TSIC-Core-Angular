...
| R-04 | ~~Add a Sort Order control to the items editor~~ — **DONE.** The edit modal writes Active + SortOrder, the only two fields `UpdateItem` honours (R-05). | CLOSED |
...
| R-06 | ~~Sort the items list by SortOrder, or offer both~~ — **DONE, both.** A By name / Storefront order toggle above the items table, defaulting to legacy alphabetical. Storefront mode also shows each item SortOrder beside its id, because a director who can EDIT that number needs to see what it did without walking to the storefront; 0 renders as an em dash, since 0 means no preference and sorts last (A-02). | CLOSED |
| R-07 | ~~Add `PickedUp` to the SKU panel~~ — **DONE.** Picked Up sits between Max and Sold, tooltipped "Paid units collected - batch signed for". | CLOSED |
| R-08 | ~~Keep legacy `UnSold` as its own column alongside In Cart~~ — **DONE.** Max · Picked Up · Sold · Unsold · In Cart · Available are all present and separately tooltipped; Unsold ignores carts, Available does not. | CLOSED |
| R-09 | ~~Preserve legacy alphabetical size ordering~~ — **DONE.** `GetAllSizesAsync` orders by `StoreSizeName`. | CLOSED |
...
| R-10 | Do **NOT** replicate B-29 (`hide.bs.modal` fires the POST, so Cancel and × also create the item). It is a legacy defect, not a feature; our modal submits only from the Create button. | STANDING |
