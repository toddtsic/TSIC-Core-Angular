...
| AmericanSelectEvaluation | **NO scaffold needed** — `Registrations`+`Families` already mapped | ◐ built — `GetAmericanSelectEvaluationRowsAsync` + `AmericanSelectReportPdfService.GenerateEvaluationAsync` (landscape, check-in box + mom contact); **live EF test PASSED vs proc** (166=166 rows); layout inferred → user visual check |
