...
### Issue 7 — Idempotency guard can never match → zero double-charge protection (VERIFIED 2026-05-21 — dead guard REMOVED 2026-05-23; redesign pending) — **HIGH**
...
**STEP 2 — redesign PENDING.** Double-charge protection is now intentionally absent until a correct, **schema-free** guard is designed (no schema change — the battle-tested tables stay as-is). The design question reduces to: *where does the per-payment key get recorded, using existing columns, so a pre-charge check can find it* — and committed before/with the charge so it also closes P1's retry window. The team path (`ChargeTeamsAsync`) needs the same guard (it has the frontend key but no DTO field/gate today).
...
|---|---|---|---|
| P1 | ✅ **VERIFIED** — charge persisted after gateway approval with no compensating action if `SaveChangesAsync` then fails | Money at ADN, no local record → customer charged, system thinks unpaid | **Confirmed.** ADN charge precedes the single `SaveChangesAsync` (player `:1343`, team `:303`); no try-catch, no void/refund on failure, no outbox → failed save = card charged, nothing booked. Team+accounting are atomic (one save on the shared scoped `SqlDbContext`, `Program.cs:393`); eCheck `Settlement` rows commit in a *second* transaction (`:328`) so they can be lost while the charge stands. Retry then double-charges via Issue 7. Fix = outbox/compensation; discuss before acting. |
...
- 2026-05-23 — Issue 7 STEP 1: removed the dead guard (commit `466553df`, 22 deletions) — both call sites + `IsDuplicateAsync` + `AnyDuplicateAsync` (repo + interface). Behavior-neutral (gate never fired); client key retained for the redesign. Build clean; suite green (444). STEP 2 (schema-free redesign) pending.
...
**Issue 7 STEP 1 done (2026-05-23):** the inert guard was removed (commit `466553df`) — behavior-neutral, client key retained. **STEP 2 is the next active work:** design a *schema-free* double-charge guard (the battle-tested tables stay as-is) that also closes P1's retry window, covering both the player and team paths.

**No other fixes applied** — Issues 2/3/4/6, P1 durability remediation, and probes P3/P4/P5/P6 each remain their own decision with explicit go-ahead.
